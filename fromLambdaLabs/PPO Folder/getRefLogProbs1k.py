...
def load_dataset_with_rewards(json_path, tokenizer_name, max_length=256):
    """
    Expected JSON format (for this script):
    ...
    [
      {
        ...
        "sentence": "...",
        "human": "...",
        "base_reply": "..."   <-- model response we care about
      },
      ...
    ]

    We only care about tokenizing the model responses (here: base_reply),
    because ref_logprobs are computed over those.

    Rewards are not present in this file, so we just create a dummy
    zero reward vector (they're unused in this script).
    """
    ...
    if not isinstance(data, list) or len(data) == 0:
        raise ValueError(f"Expected a non-empty list in {json_path}, got {type(data)}")

    # Use base_reply as the model response field for this dataset
    if "base_reply" not in data[0]:
        raise KeyError(
            "Expected key 'base_reply' in JSON items, but it was not found. "
            f"Available keys in first item: {list(data[0].keys())}"
        )

    responses = [d["base_reply"] for d in data]

    # Dummy rewards (not used, but keep shape consistent)
    rewards = torch.zeros(len(data), dtype=torch.float32)
...
def main():
    ...
    # Load dataset (responses + dummy rewards)
